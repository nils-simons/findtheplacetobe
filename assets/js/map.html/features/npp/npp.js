const plants = [
  ...
  // United Kingdom
  { name: "Heysham 1 Nuclear Power Station", coordinates: [-2.921, 54.042] },
  { name: "Heysham 2 Nuclear Power Station", coordinates: [-2.921, 54.05] },
...
const loadNPP = () => {
  plants.forEach((plant) => {
    ...
    map.on("load", () => {
      map.addSource(`${plant.name}-danger-zone`, {
        type: "geojson",
        data: createCircle(plant.coordinates, 30000), // 30km radius
      });

      map.addLayer({
        id: `${plant.name}-danger-zone-layer`,
        type: "fill",
        source: `${plant.name}-danger-zone`,
        layout: {},
        paint: {
          "fill-color": "#ff0000",
          "fill-opacity": 0.3,
  ...
  // Helper to create a circle as GeoJSON
